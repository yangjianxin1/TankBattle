...
//	private Channel channel;
	...
	public MonitorRobotThread(String roomId, String tankId) {
		this.roomId = roomId;
	...
	private boolean drawRoute(LinkedList<Tank> routes, int step, RobotTank robot) {
		...
		while (it.hasNext() && step >= 0) {
					...
					if (ironFlag == false) {
						synchronized (robot.getRemainBulletCount()) {
							if (robot.getRemainBulletCount() > 0) {
								...
								Thread thread = new Thread(new BulletRepaintThread(bullet, robot, roomId));
								thread.start();
							}
						}
			...
			// 发送位置信息
						...
						TankServer.ROOM_CHANNELGROUP.get(roomId).get(tankId).writeAndFlush(tankLocationResponse);
